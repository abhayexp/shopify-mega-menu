  ...
  export default function MegaMenuFullScreenLayout() {
    ...
    const toggleActive = () =>{
    console.log("toggle active called", tree);
    setTree([]);
    setActive(prev => !prev);
  }

  const handleAddMenu = () => {
  setIsCustomise(false);
  setCustomise(undefined);
  setTree([]); // fresh tree
  setMenu({
    id: `menu-${uuidv4()}`,
    title: '',
    ...
    return (
      <Frame>
        <div style={{ marginTop: "10px" }}>
          <Button onClick={handleAddMenu}>Add Mega Menu</Button>
        </div>

        {active && (
          <div style={{ position: 'fixed', inset: 0, background: '#fff', zIndex: 9999, display: 'flex', flexDirection: 'column' }}>
            {/* Header */}
            <div style={{ padding: 16, borderBottom: '1px solid #ccc', display: 'flex', justifyContent: 'space-between', backgroundColor: '#f6f6f7' }}>
              <strong>Mega Menu #{Math.floor(Math.random() * 100000)}</strong>
              <div style={{ display: 'flex', gap: 8 }}>
                <Button onClick={toggleActive} >Cancel</Button>
                <Button size="medium" variant="primary" onClick={handleMenuSubmit}>Save</Button>
              </div>
            </div>
            ...
            <div style={{ display: 'flex' }}>
              {/* Left */}
              <div style={{ width: '60%', borderRight: '1px solid #ccc', padding: 20, overflowY: 'auto' }}>
                <TextField
                  label="Menu Name"
                  value={menu.title}
                  onChange={(value) => setMenu(prev => ({ ...prev, title: value }))}
                  autoComplete="off"
                />
                <TreeEditor tree={tree} setTree={setTree} isCostomize={isCustomise} data={customize} />
              </div>


              {/* Right */}
              <div style={{ width: '45%', padding: 24, overflowY: 'auto' }}>
                <TreeView tree={tree} setTree={setTree} isCostomize={isCustomise} data={customize}  />
              </div>
            </div>
          </div>
        )}
